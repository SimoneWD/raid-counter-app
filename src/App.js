...
const ShardTracker = () => {
  ...
  // STATO PER INPUT MANUALE DELLE SCHEGGE
  ...
  const updateShardCount = async (playerId, shardType, increment = null, manualValue = null) => {
    let newCount;
    if (manualValue !== null) {
      newCount = Math.max(0, parseInt(manualValue) || 0);
    ...
    try {
      const response = await fetch('/.netlify/functions/updatePlayerShards', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ playerId, shardType, newCount }),
      });
      if (!response.ok) {
          const errorBody = await response.text();
      ...
      setEditingShardInput({ playerId: null, shardType: null, value: '' });
    } catch (error) {
  ...
  const addLegendaryChampion = async () => {
    if (newLegendary.name.trim() && newLegendary.player && newLegendary.shardType.trim()) {
      const championToSend = {
        ...
        player: parseInt(newLegendary.player)
      };
                  ...
                  {players.map(player => (
                    <tr key={player.id} className="border-b border-gray-700 hover:bg-gray-750 transition-colors duration-200">
                      <td className="py-2 px-2 font-medium">
                        {editingPlayer === player.id ? (
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <button onClick={saveEdit} className="text-green-400 hover:text-green-300 p-1 rounded-full hover:bg-gray-600 transition-colors" title="Salva modifiche">
                              <Save className="w-4 h-4" />
                            </button>
                            <button onClick={cancelEdit} className="text-red-400 hover:text-red-300 p-1 rounded-full hover:bg-gray-600 transition-colors" title="Annulla modifiche">
                              <X className="w-4 h-4" />
                            </button>
                          </div>
                        ) : (
                          <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => startEdit(player)} className="text-gray-400 hover:text-white p-1 rounded-full hover:bg-gray-700 transition-colors" title="Modifica nome giocatore">
                          ...
                          <div className="flex items-center justify-center gap-2">
                            {editingShardInput.playerId === player.id && editingShardInput.shardType === shard.id ? (
                              <input
                                ...
                                onBlur={() => updateShardCount(player.id, shard.id, null, editingShardInput.value)}
                                onKeyPress={(e) => {
                                  if (e.key === 'Enter') {
                              ...
                              <>
                                <button onClick={() => updateShardCount(player.id, shard.id, -1)} className="w-6 h-6 bg-red-600 hover:bg-red-700 active:bg-red-800 text-white rounded-full text-sm font-bold flex items-center justify-center transition-all duration-200 shadow-sm hover:shadow-md" title={`Diminuisci ${shard.name}`}>
                                  -
                                </button>
                                <span className="font-bold text-lg min-w-[2rem] text-center cursor-pointer" onClick={() => setEditingShardInput({ playerId: player.id, shardType: shard.id, value: player.shards[shard.id] || 0 })} title="Clicca per modificare manualmente">
                                  {player.shards[shard.id] || 0}
                                </span>
                                <button onClick={() => updateShardCount(player.id, shard.id, 1)} className="w-6 h-6 bg-green-600 hover:bg-green-700 active:bg-green-800 text-white rounded-full text-sm font-bold flex items-center justify-center transition-all duration-200 shadow-sm hover:shadow-md" title={`Aumenta ${shard.name}`}>
                                  +
                                </button>
                                <button onClick={() => updateShardCount(player.id, shard.id, null, 0)} className="w-6 h-6 bg-gray-600 hover:bg-gray-700 active:bg-gray-800 text-white rounded-full text-xs flex items-center justify-center transition-all duration-200 shadow-sm hover:shadow-md" title={`Azzera ${shard.name}`}>
                                  Ø
                                </button>
                              </>
                            )}
                      ...
                      <td className="text-center py-2 px-2">
                        <button onClick={() => deletePlayer(player.id)} className="text-red-400 hover:text-red-300 p-1 rounded-full hover:bg-gray-700 transition-colors" title="Elimina giocatore">
            ...
            <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 animate-fade-in"
              ...
              <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full shadow-2xl transform scale-95 animate-scale-up"
                ...
                <p className="text-gray-300 mb-6">
                  ...
                  {(() => {
                    const championDetailsForModal = legendaryChampions.find(c => c.id === legendaryToDelete);
                    if (championDetailsForModal?.shard_type) {
                      return (
                        <>
                          <br />
                          <span className="text-sm text-gray-400 mt-2 block">
                            Tipo Scheggia: <span className={`px-2 py-1 rounded-md text-xs font-semibold ${getShardTypeColor(championDetailsForModal.shard_type)} text-white`}>
                              {getShardTypeName(championDetailsForModal.shard_type)}
                            </span>
                          </span>
                        </>
                      );
